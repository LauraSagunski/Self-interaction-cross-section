...
def sigmainter(kappa, beta):
  if kappa < 0.5:
    ...
    return 10**f(np.log10(np.amin([kappa,kappalist[-1]])),np.log10(beta))[0,0]
...
for kappa0 in kappa0grid:
  for beta0 in beta0grid:
    ...
    xfunction = lambda x: sigmainter(kappa0 * x, beta0 / x**2.) 

    averagedsigmagrid.append([beta0, kappa0, velocity_averaging(xfunction)])
    averagedsigmagrid_wrong.append([beta0, kappa0, wrong_velocity_averaging(xfunction)])
...
plt.plot(beta0grid, np.array([averagedsigma(10.,beta0) for beta0 in beta0grid]), label='$\overline{\sigma_\mathrm{'+mode+'}}$ ($\kappa_0 = 10$)', color='xkcd:blue',zorder=2)
plt.plot(beta0grid, np.array([averagedsigma_wrong(10.,beta0) for beta0 in beta0grid]), label=r'$\langle \sigma_\mathrm{'+mode+'} v_\mathrm{rel} \\rangle / \langle v_\mathrm{rel} \\rangle$ ($\kappa_0 = 10$)', color='xkcd:blue',linestyle='--',zorder=2)
plt.plot(beta0grid, [sigmainter(4./np.sqrt(np.pi)*10.,beta0*np.pi/16.) for beta0 in beta0grid], label=r'$\sigma_\mathrm{'+mode+'}(\langle v_\mathrm{rel} \\rangle)$ ($\kappa_0 = 10$)', color='xkcd:blue',linestyle=':',zorder=2)
plt.plot(beta0grid, np.array([averagedsigma(2.,beta0) for beta0 in beta0grid]), label='$\overline{\sigma_\mathrm{'+mode+'}}$ ($\kappa_0 = 2$)', color='xkcd:green',zorder=1)

plt.legend()
plt.xlabel(r'$\beta_0$')
plt.ylabel(r'$\sigma \, m_\phi^2 / \pi$')
...
CS = ax.tricontour(np.array(mpgrid)[:,1], np.array(mpgrid)[:,0], np.array(mpgrid)[:,2], [2e-3,5e-3,1e-2,1.5e-2,2.5e-2], zorder=1)
...
ax.text(12, 0.5, r'$\kappa_\mathrm{Cluster} < 1$', fontsize=12,  color='grey')
ax.text(200, 0.04, r'$\beta_\mathrm{Cluster} < 10^{-3}$', fontsize=12,  color='grey')
...
CS = ax.tricontour(np.array(sDwarfgrid)[:,1], np.array(sDwarfgrid)[:,0], np.array(sDwarfgrid)[:,2], [6,12,30,100],zorder=1)
...
ax.text(15, 0.5, r'$\kappa_\mathrm{Dwarf} < 1$', fontsize=12,  color='grey')
ax.text(200, 0.04, r'$\beta_\mathrm{Cluster} < 10^{-3}$', fontsize=12,  color='grey')
