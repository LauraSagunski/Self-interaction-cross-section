...
beta0grid = np.logspace(-3,3, 61, endpoint=True)
...
for i, mode in enumerate(modes):
  for j, sign in enumerate(signs):
    ...
    for k in [0,1,2]:
      ...
      axs[i,j].plot(beta0grid, np.array([np.pi*averagedsigma(kappaplot,beta0,mode,sign) for beta0 in beta0grid]), color=color_list[k] ,zorder=2)
