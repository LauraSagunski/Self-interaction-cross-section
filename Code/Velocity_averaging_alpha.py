...
beta0grid = np.logspace(-4,4, 41, endpoint=True)
...
for alpha in agrid:
  ...
  for mx in mxgrid:
    ...
    bDwarfgrid.append([alpha, mx, beta0(mp,mx,alpha,v0Cluster)])
...
plt.title(r'$m_\phi = '+str(mp)+'$', fontsize=16)
plt.tight_layout()

plt.savefig(outputname_plot4+str(mp)+'.pdf')
plt.show()
